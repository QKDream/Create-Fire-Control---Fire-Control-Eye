 ...
 *
 * <p>The arrow starts at the projected target position and ends at the
 * predicted intercept point solved against the target's motion with a
 * nominal muzzle speed, so the tip is where the shot should be aimed. The
 * tip is drawn as a small circle. The radar lock and the optical lock both
 * feed this renderer; the caller decides which one is shown to avoid
 * duplicates when both locks are active.
 */
public final class LeadArrowRenderer {
     ...
     * lock frame centre) and ends at the projected intercept point.
     */
    public static void draw(
        ...
        if (a >= -1.0E-9D || discriminant < 0.0D) {
            return;
        }
        double sqrtDiscriminant = Math.sqrt(discriminant);
        double flightTicks = (-b - sqrtDiscriminant) / (2.0D * a);
        if (flightTicks <= 0.0D) {
            flightTicks = (-b + sqrtDiscriminant) / (2.0D * a);
        }
        if (flightTicks <= 0.0D || flightTicks > MAX_FLIGHT_TICKS) {
            return;
        }
        ...
        double capped = Math.min(length, MAX_SCREEN_LENGTH);
     ...
     * prefers the matching radar contact, then falls back to the entity whose
    ...
    public static Vec3 velocityFor(Vec3 point) {
        ...
        if (contacts != null) {
            for (ClientRadarState.Contact contact : contacts) {
                ...
                if (contact.center().distanceToSqr(point) <= 144.0D) {
                    return velocity;
                }
            }
        }
        ...
        Vec3 best = null;
        double bestDistanceSqr = 49.0D;
        for (Entity entity : minecraft.level.entitiesForRendering()) {
            Vec3 velocity = entity.getDeltaMovement();
            if (velocity == null || velocity.lengthSqr() < 1.0E-6D) {
                continue;
            }
            double distanceSqr = entity.getBoundingBox().inflate(1.0D).distanceToSqr(point);
